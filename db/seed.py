"""
seed.py — Loads realistic sample tickets and health check data into SQL Server.
Useful for testing reports and the escalation engine without waiting for real events.
...
def seed():
    ...
    try:
        ...
        if existing > 0:
            ...
            print("   To re-seed, delete all tickets first or use a fresh migration.")
            return
        ...
        for data in SAMPLE_TICKETS:
            ...
            db.add(TicketEvent(
                ticket_id=ticket.id,
                event_type="created",
                message=f"Ticket seeded (source: {ticket.source})",
                created_at=created_at,
            ))

            if ticket.status in ("resolved", "closed") and resolved_at:
                db.add(TicketEvent(
                    ticket_id=ticket.id,
                    event_type="resolved",
